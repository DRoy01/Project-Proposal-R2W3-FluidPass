...
async function main() {
  ...
  const FluidPass = await hre.ethers.getContractFactory("FluidPass");
  //deploy the money router account using the proper host address and the address of the first signer
  const fluidPass = await FluidPass.deploy();

  await fluidPass.deployed();

  console.log("FluidPass deployed to:", fluidPass.address);
}
